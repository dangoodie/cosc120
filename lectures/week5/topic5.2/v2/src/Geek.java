...
public class Geek {
    ...
    //EDIT 1: method to delegate outputting of contact details to Geek rather than doing it in SeekAGeek
    /**
    ...
    //EDIT 3: delegate the task of generating the String database entry to the Geek class
    //This will return a String that matches the format used in the database (the allGeeks txt file).
    /**
    ...
    public String getDatabaseEntry() {
        return this.getUsername()+","+this.getName()+","+this.getAge()+",0"+
                ...
                this.genericFeatures.getStarSign()+","+"religion"+","+
                this.getGenericFeatures().getFavouriteGames().toString().replace(", ",",")+"," +
                "["+this.getStatement()+ "]"+","+this.getBlockedGeeks();
    }

    //EDIT 5: delegate the task of describing a geek to the Geek class, so that no matter what changes are
    //made to the Geek class in the future, they're handled internally
    /**
     * @return a String describing a matching geek to the user
     */
    public String getGeekDescription(){
        return this.getName()+" is a/an "+this.getAge()+" year old "+this.genericFeatures.getStarSign()+" "+
