...
public class DreamGeek {
     ...
     * @param features the features to show (dependent on relationship)
     */
    public String getDescription(Set<Criteria> features){
        StringBuilder description = new StringBuilder();
        for(Criteria key: features) description.append("\n").append(key).append(": ").append(getCriteria(key));
    ...
    public boolean matches(DreamGeek realGeek) {
        for(Criteria key : realGeek.getAllCriteria().keySet()) {
            if(this.getAllCriteria().containsKey(key)){
                ...
                //EDIT: this allows the user to select more than one star-sign
                else if(getCriteria(key) instanceof Collection<?>){
                    Set<Object> items = new HashSet<>((Collection<?>) getCriteria(key));
                    if(!items.contains(realGeek.getCriteria(key))) return false;
                }
